/*
 * 右键菜单模块（与 sidebar.c 风格一致的深入解读）
 * 目标：提供针对“文件项/空白区域”的上下文操作入口，动作由文件系统层实现。
 * 
 * 职责（参考 sidebar 的分层组织）：
 * 1. 模型层：以 MenuItem 单链表描述菜单结构（动作项/分隔符，enabled 状态）。
 * 2. 视图层：计算尺寸、绘制背景/边框/文本/分隔线，保证在窗口内可见。
 * 3. 控制层：根据点击位置解析命中的菜单项并执行动作；ESC 或外部点击隐藏。
 * 
 * 菜单构建策略：
 * - 针对文件项：Open/Copy/Cut/Delete/Rename/Properties 等。
 * - 针对空白区：Paste（基于剪贴板可用性）、New Folder/File、Refresh。
 * - 构建前清理旧链表，避免内存泄漏；以 MENU_MIN_WIDTH 保底宽度。
 * 
 * 事件流（与 sidebar.c 的“边界命中 -> 反馈 -> 回调”一致）：
 * - 鼠标左键在菜单内：计算逻辑 y 偏移逐项命中，若命中且 enabled 则执行；随后隐藏菜单。
 * - 鼠标在菜单外：直接隐藏菜单，将事件交还其他组件（不吞掉）。
 * - 键盘 ESC：隐藏菜单。
 * 
 * 绘制顺序（与 sidebar 背景优先一致）：
 * 1) 背景 -> 2) 边框 -> 3) 逐项文本/分隔线（文本使用 TTF 渲染，以适配 DPI）。
 * 
 * 与 sidebar.c / file_list.c 的协作：
 * - 展示位置受主窗口大小约束（防越界）；
 * - 依赖 file_ops 提供的复制/剪切/粘贴/删除/重命名等动作；
 * - 对“重命名”动作，委托 file_list_view 启动内联编辑（保证 UI/输入一致性）。
 * 
 * 扩展建议：
 * - 可加入快捷键/图标渲染；
 * - 可根据文件类型动态禁用某些动作；
 * - 动作执行后可通过 FileListView 刷新视图，体验与 sidebar 的刷新语义保持一致。
 */

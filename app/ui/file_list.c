/*
 * 文件列表视图模块（与 sidebar.c 风格保持一致的深入解读）
 * 目标：以多视图（图标/列表/详细）呈现目录内容，支持选择、导航、排序、过滤与内联重命名。
 * 
 * 职责分层（对齐 sidebar 的“视图/交互/业务”划分）：
 * 1. 视图层：
 *    - 依据 ViewMode 绘制图标/列表/详细信息视图；
 *    - 统一背景、裁剪区域（clip rect）与选中高亮渲染；
 *    - 在详细视图中绘制表头与列分隔线（名称/大小/修改日期）。
 * 2. 交互层：
 *    - 鼠标滚轮滚动；点击选择，双击打开；右键回调触发上下文菜单；
 *    - 键盘导航（上下/回车/退格/F2）；
 *    - 内联重命名输入（文本/按键/光标闪烁）。
 * 3. 业务层：
 *    - 目录加载与驱动器列表加载（Windows）；
 *    - 排序（按名称/大小/类型/时间，支持“文件夹优先”与多规则；算法自适配）；
 *    - 搜索过滤（不重载目录，仅在现有列表中做可见性过滤）。
 * 
 * 生命周期：
 * - new：分配 FileListView、FileList、SortConfig，初始化绘制/输入状态与视口（viewport）。
 * - load_directory：加载路径后回调 on_directory_changed（由 main_window 连接到 toolbar）。
 * - draw：严格先设置裁剪/绘制背景，再按可见区域绘制项目，最后重置裁剪；
 * - free：释放列表/排序配置/纹理/路径/编辑缓冲等资源。
 * 
 * 坐标与布局（与 sidebar.c 对齐）：
 * - 视口 viewport 由 main_window 在 resize 时设置：x=SIDEBAR_WIDTH, y=TOOLBAR_HEIGHT；
 * - 滚动仅影响内容区域（header 不随内容滚动）。
 * 
 * 与其他模块的协作：
 * - 与 toolbar：目录变更回调用于维护历史记录；搜索接口用于动态过滤；视图切换接口用于 UI 模式切换。
 * - 与 context_menu：右键事件向上冒泡，菜单动作（如重命名）再回调本视图处理。
 * - 与 sidebar：共享统一布局常量与 UI 风格，导航到路径时与侧边栏含义一致。
 */
